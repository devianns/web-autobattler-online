...
export default function GameClient({ gameKey, roomName }: { gameKey?: string; roomName?: string }) {
  ...
  useEffect(() => {
    ...
    void fetch(endpoint, { cache: "no-store" }).then(async (response) => {
      ...
      const body = await response.json() as { state: PrototypeGameState;game?:OnlineMeta;serverNow?:string };
      const loaded = { ...body.state, combatHistory: body.state.combatHistory ?? [] };
      if (gameKey) sessionStorage.setItem("wa_active_game", gameKey);
      if (!cancelled) { setGame(loaded);setOnlineMeta(body.game??null);if(body.serverNow)setServerOffsetMs(new Date(body.serverNow).getTime()-Date.now());setConnection("ONLINE"); setBusy(false); }
  ...
  useEffect(()=>{if(!gameKey)return;const timer=window.setInterval(()=>{if(document.visibilityState!=="visible")return;void fetch(endpoint,{cache:"no-store"}).then(async(response)=>{if(!response.ok)throw new Error("poll failed");const body=await response.json() as {state:PrototypeGameState;game:OnlineMeta;serverNow:string};setGame({...body.state,combatHistory:body.state.combatHistory??[]});setOnlineMeta(body.game);setServerOffsetMs(new Date(body.serverNow).getTime()-Date.now());setConnection("ONLINE")}).catch(()=>setConnection("OFFLINE"))},1500);return()=>window.clearInterval(timer)},[endpoint,gameKey]);
  useEffect(()=>{if(!gameKey)return;const timer=window.setInterval(()=>setClockNow(Date.now()),250);return()=>window.clearInterval(timer)},[gameKey]);
