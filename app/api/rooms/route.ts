...
export async function POST(request:Request){try{const parsed=schema.safeParse(await request.json());if(!parsed.success)return json({error:"방 이름은 1~30자로 입력하세요."},400);const sessionId=await getSessionId();if(!(await consumeRateLimit({sessionId,scope:"room-create",limit:10,windowSeconds:60})).allowed)return json({error:"방 생성 요청이 너무 많습니다."},429);const user=await profile(sessionId);if(!user.nickname)return json({error:"닉네임을 먼저 설정하세요."},403);const roomId=await createRoom(sessionId,parsed.data.name);if(!roomId)return json({error:"방을 만들지 못했습니다."},409);return json({roomId},201)}catch(error){console.error("rooms.create.failed",error);return json({error:"방 생성에 실패했습니다."},500)}}
