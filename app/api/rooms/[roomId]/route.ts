...
export async function POST(request:Request,{params}:Context){try{const parsed=action.safeParse(await request.json());if(!parsed.success)return json({error:"잘못된 방 명령입니다."},400);const {roomId}=await params;const sessionId=await getSessionId();await profile(sessionId);let ok=false;
  ...
  return json({ok:true,room:await roomDetail(roomId,sessionId),serverNow:new Date().toISOString()});
