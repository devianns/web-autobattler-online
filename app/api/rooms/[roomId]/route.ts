...
export async function POST(request:Request,{params}:Context){try{const parsed=action.safeParse(await request.json());if(!parsed.success)return json({error:"잘못된 방 명령입니다."},400);const roomId=z.string().uuid().safeParse((await params).roomId);if(!roomId.success)return json({error:"잘못된 방 ID입니다."},400);const sessionId=await getSessionId();await profile(sessionId);let ok=false;
